...
除休眠壁纸外，插件还内置一个**全屏「墨痕阅读统计」面板**，可随时查看阅读概览，并支持通过 simpleui 导航栏、KOReader 手势（Dispatcher）等入口一键调出。
...
当前版本仅在 Kindle 上进行了测试，其他设备和平台尚未测试。若在其他设备上使用，建议先备份 KOReader 设置。

## ✨ 功能特性

- 📊 **墨痕账单壁纸**：根据阅读统计生成收据 / 账单风格休眠壁纸
- 📈 **墨痕阅读统计面板**：全屏阅读统计概览，可随时调出查看（详见下文）
- 🔀 **多数据源支持**：可选 KOReader 阅读统计、觅阅书架或两者合并
- ⏱️ **阅读时长统计**：显示本期累计阅读时长、日均阅读时长和合计时长
- 📚 **Top 书单展示**：按本期阅读时长展示 Top 2 / Top 3 / Top 4 / Top 5 书籍
- 📉 **每日趋势折线 / 热力图**：壁纸可显示每日阅读趋势折线，或最近 26 周（约半年）阅读热力图
- 🧾 **小票风格排版**：包含单号、统计周期、来源、条码和底部署名
- 🖼️ **自定义背景图片**：支持纯白背景，或指定一张自定义图片作为壁纸底图
- 💧 **墨水污渍效果**：在壁纸上叠加墨点聚集质感（默认关闭，避免低性能设备卡顿）
...
- 🚀 **导航栏与手势入口**：可在 simpleui 导航栏、KOReader 手势（设置 → 手势）或 ZenOS 导航栏中绑定「墨痕统计」，一键调出统计面板
- 🌐 **在线更新**：插件内一键检查 GitHub Release 新版本，支持下载并自动安装
- 🔒 **OTA 哈希验证**：下载完成后自动校验 SHA256，防止文件损坏或篡改
- 📡 **GitHub 镜像加速**：支持配置镜像源（默认关闭），国内设备可启用以获得更稳定下载
...
- 🌐 **壁纸语言**：支持简体中文、繁體中文（香港）、English 壁纸渲染，独立于 KOReader UI 语言
- 📦 **.po 语言包**：采用标准 gettext .po 文本文件，纯 Lua 运行时解析，无需编译
- 🪶 **轻量模式**：降低渲染分辨率并跳过重计算，适合低内存设备
- ✍️ **统计界面署名**：可自定义品牌字样（如「墨痕 / 书斋」），同时用于锁屏壁纸与统计面板
...
- 图表：显示每日阅读趋势折线，或最近 26 周阅读热力图
- 底部：二维码、Code128 风格条码、随机格言和 `Design by Estela-Zelin84` 署名

## 📊 墨痕阅读统计面板

除休眠壁纸外，插件内置一个**全屏「墨痕阅读统计」面板**（非壁纸图，复用锁屏账单风格）。它聚焦数据本身，包含：

- 头部统计信息（统计周期、来源、总时长等）
- 书单：最多展示 9 本书（按阅读时长置顶）
- 底部墨痕图标

面板**不含**二维码、条码、格言与图表，更侧重于即时查阅阅读概览。

**如何调出：**

| 入口 | 说明 |
|------|------|
| 插件菜单 | 顶部菜单 → 墨痕壁纸 → **查看墨痕统计** |
| simpleui 导航栏 | 在 simpleui 底部栏设置中，将「墨痕统计」加为快捷 tab |
| KOReader 手势 | 「设置 → 手势」中，把任意手势（轻扫 / 长按 / 双击等）绑定到「墨痕统计」动作（Dispatcher） |
| ZenOS 导航栏 | 在 ZenOS 导航栏「添加操作」中选择「墨痕统计」 |

**如何关闭：** 点击右上角 ✕ 关闭键、全屏向左/右滑动，或按硬件返回键。

> 注：统计面板默认展示最近一年（year）的阅读概览；其品牌署名与锁屏壁纸共用同一设置（菜单「外观设置 → 统计界面署名」）。

## 🔧 使用方法

1. 下载 release 中的 `inkstain.koplugin-v3.8.2.zip`
...
| 菜单项 | 说明 |
|------|------|
| 查看墨痕统计 | 调出全屏阅读统计面板（另可经 simpleui / 手势 / ZenOS 入口调出） |
| 生成并设为休眠壁纸 | 生成新壁纸，并设置为 KOReader 休眠屏幕图片 |
| 关闭墨痕壁纸 | 停止自动刷新，并恢复启用墨痕前的原生屏保设置 |
| 数据与统计 → 统计周期 | 今天 / 最近 7 天 / 最近 30 天 / 最近一个季度 / 最近半年 / 最近一年 / 最近 10 年 |
| 数据与统计 → 书单数量 | Top 2、Top 3、Top 4、Top 5 |
| 数据与统计 → 数据源 | KOReader 阅读统计 / 觅阅书架 / 两者合并 |
| 数据与统计 → 进度模式 | 总进度（全期阅读位置）或本期进度（本期阅读页数占比） |
| 外观设置 → 背景模式 | 纯白背景，或自定义图片背景 |
| 外观设置 → 选择 / 清除背景图片 | 设置或清除壁纸底图 |
| 外观设置 → 墨水污渍效果 | 叠加墨点质感（默认关闭） |
| 外观设置 → 图表样式 | 折线图，或热力图（最近 26 周阅读分布） |
| 外观设置 → 统计界面署名 | 自定义品牌字样，同时用于锁屏壁纸与统计面板 |
| 外观设置 → 壁纸字体 | 选择字体 / 恢复内置汇文明朝体 |
| 外观设置 → 壁纸语言 | 简体中文 / 繁體中文（香港）/ English |
| 通用设置 → 仅生成壁纸 | 只生成图片，不修改 KOReader 屏保设置 |
| 通用设置 → 休眠前自动刷新 | 当前正在使用墨痕壁纸时，休眠前自动重新生成 |
| 通用设置 → 自动设置 KOReader 休眠屏幕 | 生成后自动设置为休眠壁纸 |
| 通用设置 → 锁屏使用范围 | 只在主页使用，或主页和阅读界面都使用 |
| 通用设置 → 轻量模式（低内存设备） | 降低分辨率并跳过重计算，适合低内存设备 |
| 通用设置 → 软件更新 | 检查 GitHub Release 新版本并在线安装 |
| 通用设置 → 显示输出路径 | 查看当前壁纸输出位置 |
| 加入内测 | 加入内测 QQ 群：627525507 |
| 关于 | 显示插件版本与设计署名 |
...
- ✅ 新增**全屏「墨痕阅读统计」面板**：随时查看阅读概览，支持菜单 / simpleui 导航栏 / KOReader 手势（Dispatcher）/ ZenOS 导航栏多种入口调出，右上角 ✕、滑动或返回键关闭
- ✅ 新增**导航栏与手势入口**：向 simpleui、KOReader Dispatcher、ZenOS 注册「墨痕统计」动作，可绑定到导航栏或任意手势
- ✅ **统计周期**扩展为 7 档：新增最近一个季度、最近半年、最近一年、最近 10 年
- ✅ 新增**自定义背景图片**：壁纸底图可指定为纯白或一张自定义图片
- ✅ 新增**墨水污渍效果**：壁纸可叠加墨点聚集质感（默认关闭，避免低性能设备卡顿）
- ✅ 新增**图表样式**：壁纸趋势图支持「折线图」或「热力图（最近 26 周阅读分布）」
- ✅ 新增**统计界面署名**：可自定义品牌字样（如「墨痕 / 书斋」），同时作用于锁屏壁纸与统计面板
- ✅ 新增**轻量模式**：降低渲染分辨率、跳过二维码图片加载与墨点重计算，适配低内存设备
- ✅ **壁纸语言**新增「繁體中文（香港）」
- ✅ 新增「加入内测」菜单入口
