...
* **性能优化**:绘制层采用快筛+缓存+行级定位+时间预算(由 小红书用户 剑与奏鸣曲 提供的性能优化方案),只给真正要注音的字取屏幕盒子;无生僻字的页几乎零开销,普通页显著提速。字库与等级阈值沿用原版。
...
* **性能**:绘制层已做优化(快筛+缓存+行级定位+时间预算,由 小红书用户 剑与奏鸣曲 提供的性能优化方案),只给真正要注音的字取屏幕盒子;无生僻字的页几乎零开销,普通页(默认等级 3)通常比旧版明显更快,等级 5 全文注音时仍需逐字取盒、提速有限。若仍觉卡,可降低「标注等级」或减少注音字数。
...
![1.00](assets/image-1.png)

设置：

![1.00](assets/image-4.png)

效果：

5级：

![1.00](assets/image-5.png)

四级：

![1.00](assets/image-6.png)
