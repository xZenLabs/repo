...
I resolved a small issue with the Change PIN Dialog and made it keep the menu open for better UX.
Also, an upcoming "delete plugin settings" hook for KOReader (nightly) is now provided. This will allow you to clean any plugin settings remnants upon disabling the plugin. The disabling of the plugin will still always erase the configured PIN.
