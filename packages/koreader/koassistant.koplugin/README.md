...
**X-Ray version ladder (manual, runs in the background):** from the X-Ray popup, **"Build X-Ray version ladder…"** generates a full set of prefix versions of the book's X-Ray — roughly one every 10% of a normal-length book (spacing widens for short books, and versions snap to chapter ends when the book has a usable table of contents, so they read as "up to the end of a chapter"). After one cost-confirmation dialog, the versions are built in the background one incremental step at a time — in total roughly the cost of one full X-Ray run — while you keep reading (the book must stay open; you can cancel anytime and resume later from where it stopped). As you read past a version's position, it is silently swapped in as your live X-Ray, so the X-Ray follows your reading position with zero waiting; all versions stay browsable under **"Previous versions"**. Useful before a re-read, for book clubs, or to give a book spoiler-safe X-Rays for its whole length up front.
...
- **X-Ray Versions to Keep**: Whenever an update or redo overwrites the X-Ray, the outgoing version is archived — browse, view, or restore them via "Previous versions" in the X-Ray popup and browser menu. Sets how many are kept per book (oldest dropped first; 0 stops archiving new versions). Ladder versions are stored separately and are never trimmed by this.
