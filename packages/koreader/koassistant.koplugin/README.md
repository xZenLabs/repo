...
- **X-Ray that keeps up** → optional **Automatic X-Ray** (per-book or global: background auto-update as you read, auto-create early in a book), a **version ladder** (pre-build spoiler-safe X-Ray versions for the whole book, snapped to chapter ends, silently swapped in as you read), and **version history** (browse, restore, or delete earlier X-Ray snapshots, with a keep-count setting)
...
**Background features:** this same consent covers background runs you opt into separately — [Automatic X-Ray](#x-ray-auto-update) and [X-Ray version-ladder builds](#x-ray-version-ladder) extract text and spend tokens **without a per-request confirmation**. They re-check consent at fire time (revoking it stops them), their size is bounded by their own guards (the progress-gap window; incremental ladder steps), and the interactive truncation/size warnings don't apply: an oversized background extraction aborts the run instead of asking.
...
- **Options menu** (☰): info (model, progress, date, fiction/non-fiction/academic type, ladder provenance when a version ladder exists), **All versions (N)…** (main X-Ray only, when archived versions exist, see [X-Ray version history](#x-ray-version-history) below), delete, close
...
- From the **X-Ray popup** (open book or file browser) or the X-Ray browser's **☰ menu**, tap **"All versions (N)…"** (main X-Ray only, sections have no history). The list holds both archived *previous* versions and any [ladder](#x-ray-version-ladder) versions — which can sit *ahead* of your reading position, hence "All".
...
- It only **updates** an existing incremental X-Ray or **creates** a first one early in a book (where allowed, per above), and never touches complete, AI-knowledge, legacy, or section X-Rays. It never builds a [version ladder](#x-ray-version-ladder) — that is always an explicit, cost-confirmed action.
...
**X-Ray version ladder (manual, runs in the background):** from the X-Ray popup, **"Build X-Ray version ladder…"** generates a full set of prefix versions of the book's X-Ray — roughly one every 10% of a normal-length book (spacing adapts to book length: wider on short books so a novella isn't sliced into tiny calls, narrower on very long books so a single version never covers an oversized slice; versions snap to chapter ends when the book has a usable table of contents, so they read as "up to the end of a chapter"). After one cost-confirmation dialog, the versions are built in the background one incremental step at a time — the book is read once in total, plus a fixed per-version overhead, so somewhat more than one full X-Ray run — while you keep reading (the book must stay open; you can cancel anytime and resume later from where it stopped). As you read past a version's position, it is silently swapped in as your live X-Ray, so the X-Ray follows your reading position with zero waiting; all versions stay browsable under **"All versions"**.

The ladder serves two purposes, and both are first-class: **spoiler-safe reading** (every position gets an X-Ray that never runs ahead of you), and a **checkpointed full build** — completing a big book's X-Ray in bounded increments instead of one oversized request (a failure keeps the finished versions; resume continues from the highest). If you built the ladder for the second reason and don't want spoiler gating, tap **"Switch to complete version (100%) — instant"** in the popup's Versions & ladder group: it installs the finished 100% version as your live X-Ray for free (no rung is ever merged — each ladder version already *contains* everything below it), then offers to delete the other ladder versions (keeping them is fine too; they stay browsable). The switch is not a one-way door: while the ladder versions are kept, **"Switch back to your position (NN%) — instant"** re-installs the version at or below where you're reading, and position-tracking (promotion as you read) resumes — both switches are free and reversible. Useful before a re-read, for book clubs, or to give a book spoiler-safe X-Rays for its whole length up front.
...
> The per-book domain is one of several per-book overrides. The full set (domain, research mode, spoiler-free chat, AI Book Tools posture, web search, quick answer default, book info, Automatic X-Ray, highlight context, dictionary context, AI title/author, quiz, languages) lives in the dedicated [Book Settings](#book-settings) screen.
...
| **Automatic X-Ray** | Follow global / On / Off | One switch for background X-Ray upkeep on this book. **On** is self-contained: auto-creates the first X-Ray early in a book, then keeps it updated — regardless of the global setting. **Off** always wins. **Follow global** defers to **Automatic X-Ray (all books)**. See [Automatic X-Ray](#x-ray-auto-update). |
...
- **Automatic X-Ray (all books)**: Quietly keep every book's X-Ray up to your reading position in the background (flowing formats like EPUB only). Per-book On/Off overrides win in both directions. Spend guards: fires at most once per cooldown, only for progress gaps inside the window below, and only when WiFi is already on. Never builds a version ladder. (default: OFF)
- **Auto-create X-Ray (early in a book)**: For books following the global setting with no X-Ray yet, quietly create the first one once you've read past the minimum gap; the maximum gap still caps it. Books switched to Automatic individually (per-book On) always auto-create. (default: OFF)
...
- **Minimum Progress Gap (%)**: Don't auto-update until you've read at least this much past the X-Ray's position. Lower = more frequent, smaller updates; higher = fewer, larger ones. Also the lower edge of the auto-create window.
- **Maximum Progress Gap (%)**: Gaps bigger than this never auto-update — run the update manually from the X-Ray popup, which shows the size first. Also the upper edge of the auto-create window, and the cap on automatic ladder promotions after a position jump.
- **Cooldown (minutes)**: Minimum time between background attempts (0 = no cooldown).
- **Notify on Background Activity**: Show a brief notification when a background X-Ray update or create starts and completes, and when a ladder version is silently swapped in. Off = fully silent.
- **Ladder Versions at Chapter Ends**: When building an [X-Ray version ladder](#x-ray-version-ladder), place each version at the nearest chapter end (within a few percent) so versions read as "up to the end of a chapter"; the versions list shows the chapter names. Needs a table of contents. (default: ON)
